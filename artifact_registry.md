...
| LZ-031 | decoupling-triad-backbone | Boundary | lean-proved | src/lean4/DecouplingBackbone.lean (cross-repo Lean composition: 2 theorems + 1 enumeration def + cardinality witness. `decoupling_triad_backbone` proves every joint Triad decoupling output is in the 8-element finite list (2 × 2 × 2 = 8 — `decoupling_output_cardinality` confirms by `rfl`). Composes 3 concrete legs from home-repo canonical Lean formalisations: LavaLamp.LL002Visual.VisualOutput (via existing Lake git dep on lavalamp-hermetic@1a2534f), Lazarus.VisualSkinDecoupling.Mode (new sibling module added v0.1.29 — concrete LZ-001 formalisation: 3 theorems on the 2-state mode-vocab inductive), PharOS.Membrane.MembraneOutput (via existing Lake git dep on pharos-lean@e3eaee1). No new Lake git deps required. SECOND cross-repo Lean proof in Lazarus after LZ-028 TriadBackbone. `lake build` returns 25 jobs (was 21 at v0.1.28) zero `sorry`. Promoted from :argued to :proved at v0.1.29. | src/lean4/DecouplingBackbone.lean + src/lean4/VisualSkinDecoupling.lean (Lazarus repo, both new at v0.1.29) + lavalamp-hermetic/LL002Visual.lean (LavaLamp repo, fetched via Lake git dep pinning commit `1a2534f`) + pharos-lean/Membrane.lean (PharOS repo, fetched via Lake git dep pinning commit `e3eaee1`); LAZARUS_SPEC.md LZ-001 + LAVALAMP_SPEC.md LL-002 + PHAROS_SPEC.md PH-004 entries; TCE companion docs/triad_discovery_companion.md (triadic-coordination-engine commit a9ddfea) | :proved |
| LZ-032 | lavalamp-verify-cross-validation | Operational | example-tested | test/test_lavalamp_verify.py (17-layer integration test exercising LL-043 v4 ECDSA P-256 protocol client + auth() gating logic. §1 protocol-client verdict-code scenarios (10): ACCEPT path, REJECT path, STALE path, timestamp skew >30s → ERROR, wrong-key signature → ERROR, bad version byte → ERROR, unknown result byte → ERROR, missing socket → NOSOCK, missing pubkey → NOSOCK, malformed pubkey → ERROR. §2 auth() integration (7): ACCEPT → proceed; REJECT default → hard fail before camera; ERROR default → hard fail; STALE default → soft warn + proceed; STALE strict → hard fail; NOSOCK default → soft warn + proceed; default-parameter lock via inspect.signature. Uses real ECDSA P-256 keypair via `cryptography` so the verify path runs end-to-end. New `pip install cryptography` step in CI on every push) | face_sentinel.py `_lavalamp_query()` (Python port of pharos/src/c/pam_lavalamp/pam_lavalamp.c `try_ipc_query_v4`; same wire format — 17-byte request, 74-byte response, 33-byte SEC1-compressed pubkey, signed message = nonce||result||ts(8 LE), ECDSA P-256 verify via cryptography library) + auth() Step 1.5 cross-check + argparse `--strict-lavalamp` flag + CLI dispatch; constants block (LAVALAMP_SOCK, LAVALAMP_PUB, LAVALAMP_PROTOCOL_VERSION=0x04, LAVALAMP_TS_SKEW_S=30, etc.). Consumes LL-040 socket contract + LL-043 v4 protocol from LavaLamp repo; ports PH-009 reference client from PharOS repo. First architectural fill from TCE v0.2.12 cross-Triad pass §4 — closes the largest cross-Triad mention-graph gap (LL ↔ LZ had 83 V-tag R-edges, 0 mention edges; LZ-032 adds 3 LZ → LL mention edges + 1 LZ → PH mention edge) | :tested |

## Counts

- Total: 32
- `:proved`: 8 (LZ-016 outliers + LZ-017 liveness metric +
  ...
  LZ-031 decoupling-triad-backbone — all eight lean-proved
  hermetically via `lake build` in `src/lean4/`. LZ-028 is
  the FIRST cross-repo Lean proof in Lazarus (v0.1.24);
  LZ-031 is the SECOND (v0.1.29), reusing the existing
  lavalamp-hermetic + pharos-lean Lake git deps with a new
  sibling module VisualSkinDecoupling.lean for the LZ-001
  leg).
- `:tested`: 22 — LZ-001 through LZ-015 + LZ-019 + LZ-020 +
  LZ-021 + LZ-022 + LZ-023 + LZ-027 + LZ-032 (LZ-032 added
  at v0.1.30 as the first architectural fill from the TCE
  cross-Triad pass §4 spec-impact list — closes the largest
  cross-Triad mention-graph gap by porting LL-043 v4 ECDSA
  P-256 client from PharOS PAM into Lazarus's auth path)
- `:verified`: 0
- `:benchmarked`: 0
- `:argued`: 2 — LZ-029 lean-stack-triad-backbone added at
  v0.1.25 (second cross-Triad joint-closure entry; mirror
  entries at LavaLamp LL-047 + PharOS PH-015; promotion path
  blocked by Mathlib transitive dep from LL-006). LZ-030
  ...
  + PharOS PH-017; promotion path is a cross-Triad runtime
  integration test that also promotes PH-005 as side effect).
- `:open`: 0
